...
public class ScaleHealthBar : MonoBehaviour
{
    ...
    private void OnHealthChanged(int hitPoints)
    {
        ...
        {
            _healthBar.localScale = new Vector3(_healthBar.localScale.x - (100 - ((_health.HealthPoint - hitPoints) * 100) / MAX_HEALTH) *  _initialRectangleX / 100,
                _healthBar.localScale.y, _healthBar.localScale.z);

            if (_health.HealthPoint - hitPoints >= 50f*MAX_HEALTH/100f)
            {
                _healthBarImage.color = new Color((1 - ((_health.HealthPoint - hitPoints)*100/MAX_HEALTH * 1 / 100)) * 10, 1, 0, 1);
            }
        }    
